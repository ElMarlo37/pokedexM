...
let PokemonService = class PokemonService {
    ...
    async create(createPokemonDto) {
        ...
        try {
            ...
            if (error.code === 11000) {
                throw new common_1.BadRequestException(`El pokemon ya existe en la db ${JSON.stringify(error.keyValue)}`);
            }
            console.log(error);
            throw new common_1.InternalServerErrorException('No se puede crear un pokemon - revise el log');
        }
    }
    async findAll() {
    ...
    async capture(id) {
        return this.pokemonModel.findByIdAndUpdate(id, { isCaptured: true }, { new: true });
    }
    async release(id) {
        return this.pokemonModel.findByIdAndUpdate(id, { isCaptured: false }, { new: true });
    }
};
